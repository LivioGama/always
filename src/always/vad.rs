    ...
    loop {
        ...
        if !in_speech && !voice_activity_announced {
            ...
            if early_voice_streak >= EARLY_VOICE_MIN_FRAMES {
                // Show the listening overlay IMMEDIATELY on voice onset, even
                // with "My Voice" on. Previously the announce was deferred
                // until speaker verification, which made the mic icon appear
                // 0.2-4.3s AFTER the user started speaking (measured live) —
                // the single biggest "it's not fast" complaint. A speaker
                // MISMATCH (background media / another voice) still retracts
                // the overlay: the early-voice false-start path below and the
                // speaker-gate drop paths both emit voice_activity_ended. So
                // the user gets instant feedback; media only briefly flashes.
